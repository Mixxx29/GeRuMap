...
public class SelectToolAction extends AbstractCustomAction {

    public SelectToolAction(IPublisher publisher) {
        super(publisher);
        putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_S, 0));
        putValue(SMALL_ICON, ResourceLoader.load("selection_tool.png", ResourceType.ICON));
        putValue(LARGE_ICON_KEY, ResourceLoader.load("selection_tool_large.png", ResourceType.ICON));
        putValue(NAME, " Selection Tool ");
        putValue(SHORT_DESCRIPTION, " Selection Tool  (S) ");
    }

    @Override
    public void action(Object object) {
        ProjectView projectView = MainFrame.getInstance().getEditorWindow().getActiveProjectView();
        if (projectView != null) projectView.setSelectionToolState();
    }
